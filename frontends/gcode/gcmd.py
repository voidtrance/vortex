...
class GCodeCommand:
    gcode_regexp = regex.compile(
        r'^(?P<cmd>(?P<prefix>[GMTDgmtd])(?P<code>[0-9]+))(?:\ (?P<data>[^;]*))?(?:\ *;\ *(?P<comment>.*))?$')
    ...
    def iter_params(self, exclude=None):
        for param in self.__params:
