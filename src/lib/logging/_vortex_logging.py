...
def make_builder(header_path, link_path):
    ffibuilder = FFI()
    with open(os.path.join(header_path, "logging.h"), 'r') as fd:
        content = []
        for line in fd:
            if line[0] != '#':
                content.append(line)
    ...
    ffibuilder.set_source("_vortex_logging", '#include <logging.h>',
                          libraries=["logging"],
...
if __name__ == "__main__":
    ...
    ffibuilder.compile(tmpdir=opts.output_path, target="_vortex_logging.*", verbose=3)